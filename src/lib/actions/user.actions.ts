...
export async function createUser(user: CreateUserParams) {
  try {
    ...
    if (existingUser) {
      throw new Error("User already exists with this mail id");
    }
...
export async function verifyEmail(token: string) {
  try {
    ...
    if (!token) throw new Error("Invalid token or user not found");
...
export async function resetPassword(token: string, newPassword: string) {
  try {
    ...
    const user = await User.findById(token);
    if (!user) throw new Error("Invalid token present");
...
export async function updateCredits(userId: string, creditFee: number) {
  try {
    ...
    if (!updatedUserCredits) throw new Error("User credits update and failed");
