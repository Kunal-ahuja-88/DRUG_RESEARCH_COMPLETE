...
import { createMolecularGenerationHistory , getMolecularGenerationHistoryByUser } from "@/lib/actions/molecule-generation.actions"
...
const ModalLayout = () => {
    ...
    const [smiles, setSmiles] = useState(
        ...
        const fetchUserData = async () => {
            if(session?.user?.email) {
                try {
                    ...
                    setUserId(user._id);
                    const historyFromServer = await getMolecularGenerationHistoryByUser(
                        user._id
                    );
      ...
      const handleSubmit = async(e:any) => {
               ...
               const API_KEY = process.env.NEXT_PUBLIC_NVDIA_MODEL_API;
              ...
              try {
                const response = await fetch(invokeUrl,{
                 ...
                 if (userId) {
                    await createMolecularGenerationHistory(
                      {
                        smiles,
                    ...
                    const updatedHistory = await getMolecularGenerationHistoryByUser(userId);
